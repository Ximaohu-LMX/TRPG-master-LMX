...
状态：**本轮实现与限定验证已完成**。修订日期：2026-09-09。

基线：`fix/issue-516-accompanying-entities`，提交 `d53b44baf37c30cbcacb4394acb8af3ce40cd02d`。

本版按用户描述规划并经授权实施，保留模型根据情境判断人物态度的自由度。下文保留方案内容，随行语义修改与验证见第 7、8 节，后续入口分派修复见第 9 节，跨场景 NPC 记忆关联见第 10 节；本轮不处理模型语义误判，不追求百分百正确率。
...
## 10. 最小修复：叙事关联行动后在场 NPC 的记忆

2026-09-09，前面已完成的修复先提交为 `5a665c9`，本节对应随后单独实施的改动。

本轮范围仅为叙事的关联人物集合：原话匹配的人物或实体、明确 `@` 对象、
行动后玩家视图中的可见 NPC，合并去重后使用现有记忆检索。

在 `ActionPlanTurnApplication._narrate()` 中，读取记忆前从当前公开视图收集 NPC ID，
作为 `related_entity_ids` 传入 `_read_memory_context()`，与原有匹配结果合并。
该视图已在引擎完成移动后刷新，所以不需要额外执行 NPC 移动。
规划阶段和 Keeper 记忆检索保持原有调用方式。

继续使用已有地点关联、人物关联、受众筛选、排序和数量/字数预算。
不新增记忆类型、数据库结构、提示词或模型调用，不修改模组人物资料。
NPC 移动结果被归为 inventory、缺少随行原因的问题仍单独处理，不纳入本轮。

### 验证

新增 `test_narration_npc_memory.py` 的两个参数化用例，使用隔离数据库中的真实记忆投影与
检索、内存中的真实规则引擎及现有生产模型适配器；模型响应固定为测试数据，不访问外部模型。

- 艾米丽随行时，实际移动到庄园后，叙事模型输入包含接待大厅的同行邀请及同意台词。
- 艾米丽不随行、留在接待大厅时，庄园叙事不因她此前在场而关联这些记忆。
- 两种情况下都关联庄园当前在场 NPC 的跨地点经历；不自动加入未在场 NPC 或普通物件。
- 请求原话仅为“去莱恩庄园”，没有点名 NPC；调用仍为一次规划、一次步骤裁决和一次叙事。

新增用例与现有记忆、叙事恢复回归合计 **46 passed**（23.44 秒）。
Ruff 检查与格式检查、ty 类型检查、`git diff --check` 均通过。
验收只验证上下文正确传递，不要求真实模型每次都生成正确人物叙事。
